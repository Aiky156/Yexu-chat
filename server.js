...
io.on('connection', (socket) => {
  ...
  socket.on('recall message', async (data) => {
    ...
    try {
      ...
      if (message) {
        console.log('查找到要撤回的消息:', { 
          id: message.id, 
          type: message.type, 
          userId: message.userId,
          fileInfo: message.fileInfo ? '有文件信息' : '无文件信息'
        });
        ...
        if (message.userId === userId) {
          ...
          // 广播撤回消息给所有客户端
          console.log('广播消息撤回事件:', { messageId, userId });
          ...
          if (message.type === 'file') {
            // 获取文件信息（支持新旧两种数据格式）
            const msgFileInfo = message.fileInfo || message.file || {};
            const filename = msgFileInfo.filename;
          ...
          console.log('消息撤回请求被拒绝: 用户不是消息发送者', { 
            requestUserId: userId, 
            messageUserId: message.userId 
          });
        }
      } else {
        console.log('未找到要撤回的消息:', messageId);
      }
    } catch (error) {
