  ...
  db.run(`CREATE TABLE IF NOT EXISTS messages (
    ...
    if (err) {
      ...
      db.all("PRAGMA table_info(messages)", (err, rows) => {
        ...
        // 检查是否有recalled列
        const hasRecalledColumn = rows.some(row => row.name === 'recalled');
        if (!hasRecalledColumn) {
          console.log('添加recalled列到messages表');
          db.run('ALTER TABLE messages ADD COLUMN recalled BOOLEAN DEFAULT 0', (err) => {
            if (err) {
              console.error('添加recalled列失败:', err.message);
            } else {
              console.log('recalled列已成功添加到messages表');
            }
          });
        }
        
        // 检查是否有recallTime列
        const hasRecallTimeColumn = rows.some(row => row.name === 'recallTime');
        if (!hasRecallTimeColumn) {
          console.log('添加recallTime列到messages表');
          db.run('ALTER TABLE messages ADD COLUMN recallTime INTEGER', (err) => {
            if (err) {
              console.error('添加recallTime列失败:', err.message);
            } else {
              console.log('recallTime列已成功添加到messages表');
            }
          });
        }
      });
